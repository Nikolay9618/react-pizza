      ...
      <div className="pizza-block">
         ...
         <div className="pizza-block__selector">
            <ul>
               {
                  types.map(indexType => {
                     return (
                        <li key={indexType} className={activeType === indexType ? 'active' : ''} onClick={() => setActiveType(activeType => indexType)}>{typeNames[indexType]}</li>
                     )
                  })
               }
            </ul>
            <ul>
               {sizes.map((el, i) => {
                  return (
                     <li key={i} className={activeSizes === i ? 'active' : ''} onClick={() => setActiveSizes(i)}>{el} см.</li>
                  )
               })}
            </ul>
         </div>
